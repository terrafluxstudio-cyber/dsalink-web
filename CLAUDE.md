...
> **角色分工：** 代码探索用 Explore agent；内容策略、营销判断、架构决策由 Claude 主对话处理，Claude 直接改代码。
> **验证方式：** 代码改动后，Claude 运行 `git log --oneline -3` 和 `git diff HEAD~1 --stat` 查看实际改动，不靠截图或手动读文件。
...
> 详细职责见 `ROLES.md`（新角色加入 / 职责不清时调用）

| 角色 | 触发条件 | Skill |
|------|----------|-------|
| 产品架构师 | 新页面/新数据结构/新API/Phase规划 | `product-management:write-spec` |
| UI/UX 设计师 | 新组件/视觉改版/布局/移动端 | `design:*` |
| 运营/文案策划 | i18n新增/文案修改/数据更新 | `marketing:draft-content` |
| 高级文案/营销 | CTA优化/Hero文案/Persona/增长策略 | `marketing:draft-content` |
| SEO 专员 | 新页面/schema/内链/sitemap | `marketing:seo-audit` |

**跨角色硬规则：** 涉及 `lib/i18n.ts` 的任务必须串行，不得并行执行。
...
| 代码改动内容 | PM (Claude) | 综合各角色产出后直接改代码 |
...
| # | 原则 | 具体执行 |
|---|------|----------|
| 1 | **直接改代码** | 所有代码改动 Claude 直接修改源文件，`npm run build` 验证，push 到 main 触发 Vercel 部署 |
...
③ 各角色产出（spec / copy / design / SEO）
      ↓
④ PM 综合产出 → Claude 直接改代码
      ↓
⑤ 用户审核 → 验证（npm run build）
```
...
### CLAUDE.md 实时更新
每次 session 中确认新规则/决策 → 当场写入对应章节，不积压。
