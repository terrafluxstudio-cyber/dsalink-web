...
- 模式：`Home > [页面名]`（二级页面）或 `Home > [分类] > [页面名]`（三级以上）
- 在 `lib/seo.ts` 新增 `buildBreadcrumbSchema(path: string[])` helper
- 在每个页面的 JSON-LD 中加入 BreadcrumbList @graph item
...
**禁止收录：** `/admin`, `/recommend`（内部工具）, `/dsa`（redirect alias）, `/scores`（redirect alias）

**规则：** 每次新增面向用户的内容页，必须同步更新 `app/sitemap.ts`。

### 7.5 SEO 快速检查清单（新页面上线前）

- [ ] `generateMetadata()` 包含 title、description、keywords、canonical、openGraph（含 images）
- [ ] 页面已加入 `app/sitemap.ts` 并设置合理 priority 和 changeFrequency
- [ ] 页面有至少一个 JSON-LD schema（使用 lib/seo.ts helper）
- [ ] 页面内容有至少 2 个指向相关页的内链
- [ ] OG image 路径有效（通常使用 `/opengraph-image`）
