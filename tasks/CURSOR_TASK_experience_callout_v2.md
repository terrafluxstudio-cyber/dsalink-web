# CURSOR TASK: DSA Experience Callout 扩展为内容预览版

## 问题
当前 DsaExperienceCallout 是一张简洁卡片，内容量不足以体现该页面的价值，用户难以判断是否值得点击。
需扩展为显示 5 个关键内容点的预览版，让用户看到"里面有什么"。

## 涉及文件
- `components/DsaExperienceCallout.tsx` — 完整替换

---

## 完整替换内容

用以下代码完整替换 `components/DsaExperienceCallout.tsx`：
...
## 验证步骤
- [ ] `npx tsc --noEmit` 无报错
- [ ] 卡片显示 5 个预览要点（两列布局，桌面端）
- [ ] 移动端显示单列，底部有 "Read the guide →"
- [ ] 点击整个卡片跳转至 `/dsa-experience`
- [ ] 悬停时边框加深，右上角箭头间距加宽
